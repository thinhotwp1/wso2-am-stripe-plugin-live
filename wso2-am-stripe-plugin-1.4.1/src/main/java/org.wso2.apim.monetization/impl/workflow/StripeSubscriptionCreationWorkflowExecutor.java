...
public class StripeSubscriptionCreationWorkflowExecutor extends WorkflowExecutor {
        ...
        try (Connection con = APIMgtDBUtil.getConnection()) {
            ...
            if (monetizationSharedCustomer.getSharedCustomerId() == null) {
                ...
                if (monetizationPlatformCustomer.getCustomerId() == null) {
                    monetizationPlatformCustomer = createMonetizationPlatformCutomer(subscriber, requestOptions);
                }
        ...
        try {
            ...
            if (monetizationSharedCustomer.getSharedCustomerId() == null) {
                ...
                monetizationPlatformCustomer = stripeMonetizationDAO.getPlatformCustomer(subscriber.getId(),
                        subscriber.getTenantId());
                if (monetizationPlatformCustomer.getCustomerId() == null) {
                    monetizationPlatformCustomer = createMonetizationPlatformCutomer(subscriber, monetizationProperties);
                }
                monetizationSharedCustomer = createSharedCustomer(subscriber.getEmail(), monetizationPlatformCustomer,
                        requestOptions, subWorkFlowDTO);
            }
    ...
    public MonetizationPlatformCustomer createMonetizationPlatformCutomer(Subscriber subscriber, RequestOptions requestOptions)
            throws WorkflowException {
        ...
        try {
            ...
            params.put("customer", subscriber.getId());
            ...
            if (paymentMethods.isEmpty()) {
                throw new WorkflowException("Payment source is required for production monetization.");
            }
            customerParams.put(StripeMonetizationConstants.CUSTOMER_SOURCE, paymentMethods.get(0).getId());
//            customerParams.put(StripeMonetizationConstants.CUSTOMER_SOURCE, paymentMethods);
